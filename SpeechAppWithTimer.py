...
class SpeechApp(QWidget):
    ...
    def analyze_text(self, text):
        print(text)
        commands = [
            (r"(запусти(?:ти)?|запуск|постав)(?: таймер| таймера)?(?: на)? (\d+)(?: хвилин)?", self.handle_start_timer),
            ...
            (r"(відкрий|відкрити) ексель(?: файл)?", self.handle_open_excel_file),
    ...
    def create_excel_file(self, group_name):
        self.excel_file_path = f"{group_name}.xlsx"
    ...
    def open_file(self, file_name):
        ...
        try:
            ...
            for file in files:
                ...
                if name.lower() == file_name_without_extension.lower():
                    full_path = os.path.join(current_directory, file)
                    os.startfile(full_path)
                    return
            self.text_to_speech(f"Файл з іменем {file_name} не знайдено.")

        except Exception as e:
            self.text_to_speech(f"Помилка при спробі знайти файл: {e}")
    ...
    def start_timer(self,minutes):
        ...
        self.text_to_speech(f"таймер запущено на {minutes} хвилин час завершення {formatted_end_time}")
